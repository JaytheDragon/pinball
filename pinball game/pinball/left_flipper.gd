...
const ROTATION_SPEED_DOWN = 1600 # speed returning to original position
const ROTATION_LIMIT_UP = -30   # upper rotation limit
...
func _physics_process(delta):
	if Input.is_action_just_pressed("ui_left_flipper"):
		flipper_sfx.play()
	if Input.is_action_pressed("ui_left_flipper"):
		if rotation_degrees > ROTATION_LIMIT_UP:
			rotation_degrees += ROTATION_SPEED_UP * delta
			rotation_degrees = max(rotation_degrees, ROTATION_LIMIT_UP)
	else:
		if rotation_degrees < ROTATION_LIMIT_DOWN:
			rotation_degrees += ROTATION_SPEED_DOWN * delta
			rotation_degrees = min(rotation_degrees, ROTATION_LIMIT_DOWN)
