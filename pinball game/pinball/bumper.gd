...
@export var bump_force: float = 2000
@onready var sfx_player = $BumperSFX

func _ready():
	sfx_player.stream = load("res://Audio/bumper.wav")
...
func _on_body_entered(body):
	if body is Ball:
		...
		# Award points
		var awarded_points = 50
		get_parent().normal_point()
